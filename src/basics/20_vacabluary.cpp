...
void find_words(std::vector<std::string> &words, std::string_view str,
...
void find_words(std::vector<string> &words, string_view text,
                std::string_view separators) {
    ...
    while (start != string_view::npos) // Find the words
    {
        ...
        words.push_back(
                std::string{text.substr(start, end - start)}); // Store the word
        // Or: words.emplace_back(text.substr(start, end - start));    // (in-place
        // construction)
        start = text.find_first_not_of(separators,
                                       end + 1); // Find 1st character of next word
    }
}
